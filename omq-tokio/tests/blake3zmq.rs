...
async fn blake3zmq_multiple_messages_keep_counter_synced() {
    ...
    for i in 0..50u32 {
        ...
        let got = tokio::time::timeout(Duration::from_secs(1), server.recv())
            ...
            .unwrap()
            .unwrap();
        assert_eq!(got.part_bytes(0).unwrap(), msg.as_bytes());
    }
}
...
async fn blake3zmq_long_payload_uses_long_frame_aad() {
    ...
    let m = tokio::time::timeout(Duration::from_secs(1), server.recv())
        ...
        .unwrap()
        .unwrap();
    assert_eq!(m.part_bytes(0).unwrap(), &b"short"[..]);
    ...
    let m = tokio::time::timeout(Duration::from_secs(1), server.recv())
        ...
        .unwrap()
        .unwrap();
    assert_eq!(m.part_bytes(0).unwrap().to_vec(), plaintext);
    ...
    let m = tokio::time::timeout(Duration::from_secs(1), server.recv())
        ...
        .unwrap()
        .unwrap();
    assert_eq!(m.part_bytes(0).unwrap(), &b"short again"[..]);
}
...
async fn blake3zmq_multipart_message_delivered_atomically() {
    ...
    let got = tokio::time::timeout(Duration::from_secs(1), server.recv())
        ...
        .unwrap();
    assert_eq!(got.len(), 3);
    assert_eq!(got.part_bytes(0).unwrap(), &b"part1"[..]);
    assert_eq!(got.part_bytes(1).unwrap(), &b"middle"[..]);
    assert_eq!(got.part_bytes(2).unwrap(), &b"final"[..]);
