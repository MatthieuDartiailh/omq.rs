...
DRAW_ORDER = ["zmq.rs", "libzmq", "omq-compio", "omq-tokio"]
LEGEND_ORDER = ["libzmq", "omq-compio", "omq-tokio", "zmq.rs"]
...
def generate_svg(data: dict[int, dict[str, float]]) -> str:
    ...
    present = [s for s in DRAW_ORDER if any(s in data.get(sz, {}) for sz in sizes)]
    n = len(sizes)
    ...
    L.append(f'  <text x="{mid_x:.1f}" y="22" text-anchor="middle" fill="#111827"'
             f' font-size="14" font-weight="700">REQ/REP latency: TCP loopback (p50)</text>')
    ...
    # Legend
    leg_y = y_bot + 38
    legend = [s for s in LEGEND_ORDER if s in present]
    n_leg = len(legend)
    leg_spacing = plot_w / max(n_leg, 1)
    for i, name in enumerate(legend):
        lx = x_left + int(i * leg_spacing + leg_spacing * 0.2)
